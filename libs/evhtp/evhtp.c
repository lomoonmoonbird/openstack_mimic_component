...
htp__connection_readcb_(struct bufferevent * bev, void * arg)
{
    ...
    int msg_type = 0;
    if(c->type == evhtp_type_client&&c->request->method==htp_method_HEAD) 
    {
        //printf("*****************来自客户端 header\n");
        msg_type = 1;
    }
    else msg_type = 0;
    nread = htparser_run(c->parser, &request_psets, (const char *)buf, avail,msg_type);
    //printf("htparser_run nread = %zu\n", nread);
...
evhtp_connection_free(evhtp_connection_t * connection)
{
    ...
    if (connection->bev)
    {
        ...
        {
            SSL_set_shutdown(connection->ssl, SSL_RECEIVED_SHUTDOWN|SSL_SENT_SHUTDOWN);
